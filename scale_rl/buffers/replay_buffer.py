...
class ReplayBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self._sample_seed = jax.random.split(self._rng, m)   # shape (m, 2)
    ...
    def add(self, timestep: Batch) -> None:
        ...
        if len(self._n_step_transitions) >= self._n_step:
            ...
            new_seeds = jax.random.split(subkey, self._add_batch_size)
            self._sample_seed = self._sample_seed.at[add_idxs].set(new_seeds)
    ...
    def sample(self) -> Batch:
        ...
        batch['obs_seed'] = self._sample_seed[sample_idxs]
        return batch
